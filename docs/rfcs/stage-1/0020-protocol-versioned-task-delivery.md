...
For Streamable HTTP, the service reads transport headers directly from the incoming `http::Request<bytes::Bytes>`; no rmcp request extension or session carrier participates. Before any base or extension method dispatch, it validates the origin and content contract, requires exactly one `MCP-Protocol-Version` equal to the body observation, and requires exactly one textual `Mcp-Method` equal to the decoded JSON-RPC `method`. It additionally requires exactly one textual `Mcp-Name` equal to `params.name` for `tools/call` and `prompts/get`, or to `params.uri` for `resources/read`. Missing, repeated, malformed, or unequal standard routing headers return HTTP 400 with the locked core `-32001` / `HeaderMismatch` error before route lookup, task creation, access policy, planning, or application work. Header names compare case-insensitively and these values compare case-sensitively. Once a well-formed body protocol version and any required matching HTTP header have been established, a version other than the compiled `2026-07-28` target returns HTTP 400 with the locked core `-32004` / `UnsupportedProtocolVersionError`, whose data is exactly `{"supported":["2026-07-28"],"requested":"<observed version>"}`. After header and version validation, an unsupported JSON-RPC method returns exact `-32601` / `Method not found`; Streamable HTTP uses status 404 and stdio has no HTTP status. Every one of these failures precedes task creation, access policy, planning, or application work. `serve_stdio` otherwise applies the same body dispatcher to newline-delimited JSON-RPC. Clones of the HTTP service share the one finalized server instance and its RFC 0020 runtime pair.

These spellings are deliberately frozen to the locked RC source at `9d700ed62dcf86cb77475c9b81930611a9182f46`. In that source the sentinel form is specified only for custom `Mcp-Param-*` values, so standard `Mcp-Name` remains the textual routing value above; `HeaderMismatch` and `UnsupportedProtocolVersionError` are `-32001` and `-32004`. An untagged draft revision after that commit that extends sentinel encoding to `Mcp-Name` or renumbers either error is a normative delta, not implementation authority for this RFC. If the final tag contains such a delta, it cannot produce the checked-in `finalRelease` seal; the managed RFC, frozen fixtures, and acceptance contract must return to design review before implementation or publication proceeds.
...
Extension negotiation begins only after the base ingress has validated the request's `io.modelcontextprotocol/protocolVersion` and, for Streamable HTTP, its matching `MCP-Protocol-Version` header against the compiled `2026-07-28` target. It then reads the request's `io.modelcontextprotocol/clientCapabilities` metadata and validates its `extensions["io.modelcontextprotocol/tasks"]` member as the exact supported empty capability object. Unknown metadata siblings remain available to their owners. A missing or malformed body protocol-version observation is invalid parameters, a well-formed unsupported observation returns exact `-32004` / `UnsupportedProtocolVersionError`, and an unequal HTTP header returns exact `-32001` / `HeaderMismatch`; each creates no task. A malformed extension capability is invalid parameters at the same pre-task boundary. The server advertises the same fixed empty extension capability through `server/discover`; discovery never establishes remembered client capability or changes the compiled target.
...
For `tasks/get`, `tasks/update`, and `tasks/cancel` over Streamable HTTP, the client routes the request with exactly one `Mcp-Name` header whose decoded value equals the already-decoded `params.taskId`. `StatelessMcpHttpService` reads that header from the incoming `http::Request` parts alongside the standard `Mcp-Method` validation, after decoding the body but before request-local capability parsing, access-scope derivation, or store access. A missing, repeated, non-text, or unequal task name returns HTTP 400 with the locked core `-32001` / `HeaderMismatch` error and performs no capability, access-provider, or store operation. No rmcp request extension or session carrier participates. Stdio and other non-HTTP transports perform no `Mcp-Name` check for extension task methods.
...
- Design, implementation, and tests may use the locked RC, but publication remains disabled until the checked-in final-release seal proves byte equality with the official tag; a normative delta cannot produce that seal.
...
1. Land the frozen legacy/core/extension corpus, importer/check mode, optional final-release seal, offline release validator, and tamper/reproduction tests. Keep public stateless construction sealed while the checked-in evidence is RC-only.
...
- Streamable HTTP extension fixtures require exactly one textual `Mcp-Method` equal to the body method and one textual `Mcp-Name` equal to `params.taskId` for each task method, read directly from the incoming HTTP request. Missing, repeated, malformed, and unequal task names return HTTP 400 with `-32001` / `HeaderMismatch` before capability parsing, access, or storage; stdio fixtures need no task-name header. Redaction probes prove neither header nor raw task id enters framework projections or logs.
