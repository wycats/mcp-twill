...
The initial schema dialect is the closed JSON Schema 2020-12 subset needed by VBL's shipped input contract: primitives including integer, enums and constants, closed or typed-open objects, arrays, local definitions and references, and provably disjoint property-level `oneOf`. Unsupported composition and remote references fail server construction.
...
That boundary leaves real application contracts inexpressible. VBL's 63-operation baseline includes top-level integers, non-empty strings, string arrays, arrays with `minItems`, typed string maps, nested records, and arrays whose items are discriminated record unions. An audit of the generated catalogs finds exactly these input-schema keywords across all 63 baseline and 27 hybrid tools: `type`, `properties`, `required`, `additionalProperties`, `enum`, `const`, `items`, `minItems`, `minLength`, `oneOf`, and `description`. The baseline contains 21 property-level unions. `ArgType::Json` can carry those values but advertises no shape and accepts any JSON. `ArgType::Number` accepts fractional values where VBL requires an integer. Hand-writing a richer MCP schema in the adapter would make the serving surface stricter than the planner and recreate the duplicate authority RFC 0015 is intended to remove.
...
Root and branch boolean schemas, remote or recursive references, `$dynamicRef`, `anyOf`, `allOf`, `not`, conditional schemas, numeric bounds or divisibility, regular-expression patterns, maximum/uniqueness assertions, property-count assertions, `unevaluatedProperties`, tuple-array keywords, content keywords, custom vocabularies, and unknown assertion keywords fail registration. Annotation keywords unknown to the validator may be preserved only when explicitly classified as annotations and excluded from validation semantics.

This boundary is evidence-driven rather than a claim that the omitted JSON Schema vocabulary is unimportant. The released VBL input corpus uses no omitted assertion. A later adopter can add one keyword with its planner, stable redacted diagnostic, canonicalization, client-projection, and acceptance evidence instead of requiring the first implementation to standardize semantics it cannot yet test against a real public surface.
...
Numeric literals in `const` or `enum` must be represented exactly by RFC 8785's I-JSON number domain. Registration rejects an out-of-domain integer or decimal rather than allowing a later native/effect-lane snapshot compiler to round it while establishing cross-language identity. Runtime number values remain governed by the declared type and assertions; this restriction applies to schema literals that become contract bytes.
...
The compiler rejects statically empty argument domains it can express in the supported subset: an empty enum, an impossible `required`/closed-property combination, and contradictory `const`, enum, or primitive-type facts. Boolean `false` remains valid only as `additionalProperties: false`. A command cannot publish a required argument or union branch that no caller can ever satisfy.
...
2. validate the supplied JSON value against the canonical argument schema;
3. apply specialized named-type, path, workspace, or resource-reference validation when that argument kind owns it;
4. record the bound value and any named variant or schema branch identity;
5. derive the invocation fingerprint from the completed plan.
...
Registration compares the canonical validation-semantic schema produced by the Rust argument type with the assembled command input schema after removing annotations and framework-derived carrier properties. It first normalizes the *derived Rust schema only*: a two-member nullable `type` array receives canonical non-null/null order, and `anyOf: [T, {type: "null"}]` becomes `oneOf` only when `T` provably rejects null. If the authored property rejects null, Twill may then remove exactly that derived top-level null alternative. The resulting schema, object required set, and every other assertion/applicator must be canonically equal. This admits a Rust deserializer that accepts a harmless null superset of planner-validated values without making `null` part of the public contract; it also lets an explicitly nullable authored `oneOf` match the equivalent Schemars `Option<T>` representation. It does not add authored `anyOf` to Twill's dialect or attempt general schema subsumption. An ambiguous nullable `anyOf`, or any residual difference, fails registration. If comparison passed but typed deserialization later fails, Twill returns a redacted `ArgumentContractViolation`. That is an author/framework defect, not caller repair. A plain typed handler cannot be registered for a command with constrained arguments; it must opt into checked constrained extraction or dynamic validated values.
...
- **Operation catalog.** Referenced declarations appear once under `argumentSchemas`; each argument records its schema use. Inline schemas remain on their argument. All participate in catalog identity.
- **MCP schema.** Named declarations are inlined at the argument property site. Reachable same-name local `$defs` are deduplicated only when canonically identical; conflicts fail before projection, and names are never silently rewritten. A command's top-level input remains `{type: object, properties, required, additionalProperties: false}` and never gains a whole-input union.
...
- **Help.** Scalar constraints render inline. Named schemas render once with their nested properties and branches, then commands reference the name.
...
- Catalog identity changes when any validation-semantic schema fact changes.

### Implementation Phases

1. Add integer arguments, schema declarations and uses, JSON Schema 2020-12 subset compilation, canonicalization, and registration validation.
2. Integrate schema validation and redacted diagnostics into planning, including branch identity and ambiguity rejection.
...
5. Import all 63 VBL per-operation input schemas as the authoritative source fixture. RFC 0015 later owns direct/grouped native projection, consuming these canonical property schemas without reimplementing validation.

### Acceptance Tests

Acceptance lives in `crates/mcp-twill/tests/argument_schemas.rs`; shared JSON Schema compiler cases are table-driven with RFC 0014's result fixtures. The RFC 0017 source fixture consumes `baseline-tools.json` from RFC 0015's already-landed evidence-only fixture bootstrap; the new `ArgumentSchemaDecl` authoring lives in `crates/mcp-twill/tests/support/vbl.rs` and must reproduce all 63 released per-operation input schemas. The bootstrap supplies only provenance-checked observations, while RFC 0015's later public implementation separately owns native projection comparison with `surface-catalog.json`.
...
- Legacy `ArgSpec` and `ResourceDecl` JSON without `schema`/`referenceSchema` and explicit `None` values normalize to byte-identical catalog data and hash input; coarse argument and resource-carrier schemas remain unchanged until adopted.
...
- Non-empty `schemaMatch` plans serialize `instancePointer`, `oneOfPointer`, and `branchPointer` in camel case, while diagnostic keywords and contract reasons use their exact lower-snake-case spellings. JSON Schema fixtures and round trips reject accidental Rust field or variant spellings.
...
- Table-driven fixtures enforce every supported primitive, `minLength`, array, and object assertion at its boundary; typed `additionalProperties` and `minItems` project and validate identically.
...
- `handle_constrained` rejects a Rust argument schema that drifts from the command declaration as construction-only `DerivedSchemaDrift` before registry publication, handler invocation, or framework-event creation. `Option<T>` matches an absent-but-non-null Twill property only through the exact derived-null normalization, an authored nullable property retains exact null semantics, reversed nullable `type` order normalizes identically, and ambiguous derived `anyOf` fails. After successful registration, a deliberately inconsistent deserializer becomes the only served reason, `TypedDeserializationFailed`, under `ArgumentContractViolation`/`Failed`. Public details name only the catalog operation, optional attributable argument, and stable reason; Rust type/module names and serde text are absent from ordinary, RFC 0020 deferred, generated-host, event, and framework-log projections.
...
- Catalog, help, MCP schema, and fingerprint identity change with semantic constraint changes and remain stable across declaration order.
...
- The VBL fixture represents all 63 ungrouped operation input schemas, including the measured 21 property-level unions and every keyword in the generated corpus. It publishes canonical per-operation schema inputs; RFC 0015 owns grouped assembly and RFC 0016 owns the final ambient-binding requiredness needed for canonical 27-tool parity.
...
No architectural questions remain for the initial argument-schema boundary. The builder and declaration names in this body are the current Stage-0 proposal; any review-driven rename must amend the managed RFC before Stage 1, and implementation may not add an alternate authoring path. Such a revision must retain the measured schema vocabulary, explicit checked migration for legacy typed handlers, automatic checking for result-aware handlers, and serialized redacted branch selections.
