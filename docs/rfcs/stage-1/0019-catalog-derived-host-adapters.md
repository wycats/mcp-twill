...
The generated user description preserves the native tool's authored base description without appending derived catalog, resource-binding, or host guidance. This keeps the user-facing copy compatible with a released host manifest while allowing the model-facing contract to remain complete. The generated model description starts with the complete compiled native description, including RFC 0011 catalog guidance and active RFC 0016 resource-binding guidance, then appends the common host suffix and optional operation suffix. A model description may therefore intentionally differ from an archived host manifest even when its user description remains byte-identical; the derived addition is hash-covered and asserted explicitly rather than treated as fixture drift. Server instructions combine the active surface instructions with `server_prefix`. All text participates in the host adapter hash.
...
- Display names, user descriptions, browser icon, server instructions, and four prompt-reference aliases match the established extension manifest. Each model description preserves the released base description, includes the current compiled RFC 0011 and RFC 0016 guidance exactly once, and then appends the declared host suffix; the resulting intentional delta from the frozen v0.4.9 `modelDescription` is asserted explicitly.
