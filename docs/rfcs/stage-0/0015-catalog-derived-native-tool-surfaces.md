...
4. Collect RFC 0017 `requiresArguments` edges from every member. For each trigger name used by any member, compare the canonical target set of every member whose active direct schema exposes that trigger — meaning the trigger property name appears in the member's active direct-tool property union (i.e., it would appear under `properties` after step 2); a member that exposes the property without an edge contributes the empty set. All such sets must be identical, while members that do not expose the trigger — because they lack the property entirely — do not participate in the set comparison. A member that lacks the property entirely is not the same as a member that has the property but declares no edge: the former is absent from the comparison, while the latter contributes an empty set and forces all other participating members to also declare no edge. A mismatch fails grouping because the flat input object cannot condition one shared property name's relationship semantics on the selected member without whole-input composition. Repeated identical edges deduplicate canonically, and every trigger and target must be present in the grouped property union. The compiler reconstructs the common relationship map from catalog edges rather than copying a member schema's `dependentRequired` object. Version 1 emits each non-empty common set under sorted legacy `dependencies`, matching the pinned VBL compatibility surface, while the source catalog retains its draft-2020-12 `dependentRequired` projection.
