...
For `tasks/get`, `tasks/update`, and `tasks/cancel` over Streamable HTTP, the client routes the request with exactly one `Mcp-Name` header whose decoded value equals the already-decoded `params.taskId`. `StatelessMcpHttpService` reads that header from the incoming `http::Request` parts alongside the standard `Mcp-Method` validation, after decoding the body and request-local capability but before access-scope derivation or store access. A missing, repeated, non-text, or unequal task name returns `-32602` / `Invalid task routing` and performs no access-provider or store operation. No rmcp request extension or session carrier participates. Stdio and other non-HTTP transports perform no `Mcp-Name` check for extension task methods.
