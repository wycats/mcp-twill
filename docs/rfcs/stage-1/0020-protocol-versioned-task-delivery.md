...
For Streamable HTTP, the service reads transport headers directly from the incoming `http::Request<bytes::Bytes>`; no rmcp request extension or session carrier participates. Before any base or extension request dispatch, it validates the origin and content contract, requires exactly one `MCP-Protocol-Version` equal to the body observation, and requires exactly one textual `Mcp-Method` equal to the decoded JSON-RPC `method`. It additionally requires exactly one `Mcp-Name` for `tools/call` and `prompts/get`, or for `resources/read`; when the exact lowercase `=?base64?...?=` sentinel is present, Twill strictly decodes standard Base64 and UTF-8 before comparing the value to `params.name` or `params.uri`. A stateless tool may preserve the final core's typed `x-mcp-header` property annotation in its input schema. Stateless construction rejects an empty, non-token, case-insensitively duplicate, non-primitive, or non-statically-reachable annotation. For `tools/call`, each recognized non-null annotated argument requires exactly one matching `Mcp-Param-{Name}` header; Twill applies the final core's plain/Base64 decoding, safe-integer, and exact-value rules, while an absent or null argument requires the header to be absent. Unknown parameter headers are ignored. Notifications require the protocol header but do not require `Mcp-Method`, `Mcp-Name`, or parameter headers and receive no response body. Missing, repeated, malformed, undecodable, or unequal required routing headers return HTTP 400 with final core `-32020` / `HeaderMismatch` before route lookup, task creation, access policy, planning, or application work. Header names compare case-insensitively and decoded values compare case-sensitively. Once a well-formed body protocol version and any required matching HTTP header have been established, a version other than the compiled `2026-07-28` target returns HTTP 400 with final core `-32022` / `UnsupportedProtocolVersionError`, whose data is exactly `{"supported":["2026-07-28"],"requested":"<observed version>"}`. After header, metadata, and version validation, an unsupported JSON-RPC method returns exact `-32601` / `Method not found`; Streamable HTTP uses status 404 and stdio has no HTTP status. Every one of these failures precedes task creation, access policy, planning, or application work. `serve_stdio` otherwise applies the same body dispatcher to newline-delimited JSON-RPC. Clones of the HTTP service share the one finalized server instance and its RFC 0020 runtime pair.
