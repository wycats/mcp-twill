...
4. Collect RFC 0017 `requiresArguments` edges from every member. Repeated edges deduplicate canonically; every trigger and target must be present in the grouped property union. The compiler reconstructs this relationship map from catalog edges rather than copying a member schema's `dependentRequired` object. Version 1 emits the resulting sorted map under legacy `dependencies`, matching the pinned VBL compatibility surface, while the source catalog retains its draft-2020-12 `dependentRequired` projection.
...
- Grouped argument-presence relationships come only from RFC 0017 catalog edges, retain planner enforcement after selector removal, and use the version-1 native snapshot's exact legacy `dependencies` spelling without becoming a second authoring surface.
...
- A grouped tool exposes a required selector, unioned top-level properties, common required fields, `additionalProperties: false`, property-level unions from RFCs 0008 and 0017, and every catalog-declared argument-presence edge under the exact version-1 `dependencies` spelling.
