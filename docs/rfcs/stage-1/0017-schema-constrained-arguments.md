...
5. Import all 63 VBL per-operation input schemas as the authoritative source fixture. Canonicalize them under Twill's schema rules and reproduce every released property and root fact. In raw direct-schema comparison, 60 operations remain byte-identical; `fill_form` and `wait_for` differ only because Twill sorts their set-like nested `required` arrays; and `screencast_start` adds the catalog-owned `dependentRequired` map as the one semantic Twill adoption delta from the ungrouped release schema. RFC 0015 later owns direct/grouped native projection, consuming these canonical property schemas without reimplementing validation.

### Acceptance Tests

Acceptance lives in `crates/mcp-twill/tests/argument_schemas.rs`; shared JSON Schema compiler cases are table-driven with RFC 0014's result fixtures. The RFC 0017 source fixture consumes `baseline-tools.json` from RFC 0015's already-landed evidence-only fixture bootstrap; the new Twill argument authoring lives in `crates/mcp-twill/tests/support/vbl.rs`. Comparison is exact for every released property schema and root fact after applying Twill's documented canonicalization and accounting separately for the catalog-owned presence map. Raw direct-schema comparison proves that 60 operations remain byte-identical, `fill_form` and `wait_for` differ only by sorted set-like nested `required` arrays, and `screencast_start` differs only by the sorted symmetric `dependentRequired` map for `max_width` and `max_height`. The first two differences are canonical spelling changes with identical validation semantics; the one documented adoption addition captures the grouped release schema and broker behavior that the ungrouped baseline omitted. The bootstrap supplies only provenance-checked observations, while RFC 0015's later public implementation separately owns native projection comparison with `surface-catalog.json`.
...
- The VBL fixture represents all 63 ungrouped operation input schemas, including the measured 21 property-level unions, five inclusive minima, five inclusive maxima, and two integral divisors. Twill reproduces every released property schema and root fact under its documented canonicalization. Raw comparison proves 60 command schemas byte-identical to the release baseline; `fill_form` and `wait_for` differ only by sorted set-like nested `required` arrays; and `screencast_start` differs only by the exact symmetric `dependentRequired` map. Those edges capture the released grouped schema and broker rule that the ungrouped baseline omitted; RFC 0015 owns their exact grouped `dependencies` projection, and RFC 0016 owns the final ambient-binding requiredness needed for canonical 27-tool parity.
