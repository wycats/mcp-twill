...
The checked-in wire corpus at `crates/mcp-twill/tests/fixtures/mcp/tasks/` pins the external contracts separately from Twill's compiled projection. Its `manifest.json` has `formatVersion: 2` and records three exact sources: legacy MCP `2025-11-25` at commit `38c84e9f93ad191d9eb26d92b945d17bd0efcaf3`, the reviewed pre-tag core `2026-07-28` candidate at commit `31eefec6b979b09ab2092490e2271c0eb38ccd38`, and final SEP-2663 plus the Tasks Extension normative contract at commit `8966bea9c4f4e6d71060cc8284a539086e9e234f` in the canonical `modelcontextprotocol/ext-tasks` repository. It also records the extension identifier. Each payload entry contains its relative path, lowercase SHA-256, derivation (`sourceCopy` or `reviewedVector`), and one or more source records with exact source ids and normalized relative paths. Multi-authority vectors therefore record both the Tasks Extension behavior and the final core error identity they inherit. The manifest does not hash itself, lists every other file exactly once, sorts sources, paths, and source records lexicographically, and rejects absolute or escaping paths.

These fixtures are external observations, not Twill declarations or generated output. A `sourceCopy` preserves exact source bytes; a `reviewedVector` records an exact request or response case whose relationship to the pinned normative text requires human review. Normal tests verify directory membership and hashes before semantic assertions and never fetch the network. Refresh resolves archived local repositories at the authored commits and produces a visible manifest and fixture diff. The pinned pre-tag commit is implementation authority only while the public serving seal remains closed; the official tag and its versioned normative bytes remain release authority.

Serving and publication remain fail closed until final provenance exists. The manifest's optional `finalRelease` object is absent for the pre-tag bundle. After the official `2026-07-28` tag appears, the importer may add exactly `{"tag":"2026-07-28","peeledCommit":"<40 lowercase hex>"}` only after comparing every pinned candidate input with its exact versioned final path and proving byte equality. The same checked-in seal gates the public adapter: while it is absent, `CliMcpServer::into_stateless_service` returns the additive construction error `FrameworkError::ProtocolReleaseUnsealed` before invoking any task store, access provider, binder, authorizer, bridge, handler, or transport. There is no public override; crate-private tests inject an equivalent sealed/unsealed evidence source only to exercise both branches.

The offline `cargo xtask validate-mcp-task-release` command independently rejects an absent or malformed object, a non-canonical tag or peel, any fixture/hash mismatch, and any attempted release whose enabled protocol target is not that sealed revision. The crate publication workflow must pass that command. Implementation and conformance tests can therefore land against the pinned candidate while every source build and release artifact remains unable to serve under the final version string before the final tag is sealed. A byte-identical final tag changes provenance and enables the already-reviewed adapter; a normative delta leaves `finalRelease` absent and returns to design review before any fixture, accepted behavior, delivery variant, snapshot version, task runtime contract, or release changes. The unversioned extension URL can therefore never silently redefine `TasksExtension` version 1.
...
These spellings follow the reviewed pre-tag final-core candidate at `31eefec6b979b09ab2092490e2271c0eb38ccd38`: `HeaderMismatch` is `-32020`, `MissingRequiredClientCapability` is `-32021`, `UnsupportedProtocolVersionError` is `-32022`, and standard `Mcp-Name` accepts the strict Base64 sentinel. The final core allocation governs the shared missing-capability error identity while the Tasks Extension continues to govern the exact `requiredCapabilities.extensions["io.modelcontextprotocol/tasks"] = {}` payload. The official final tag remains release authority; any differing normative byte prevents the checked-in `finalRelease` seal and returns the managed RFC, frozen fixtures, and acceptance contract to design review.
...
- Design, implementation, and tests may use the pinned pre-tag candidate, but publication remains disabled until the checked-in final-release seal proves byte equality with the official tag; a normative delta cannot produce that seal.
...
1. Land the frozen legacy/core/extension corpus, importer/check mode, optional final-release seal, offline release validator, and tamper/reproduction tests. Keep public stateless construction sealed while the checked-in evidence is pre-tag.
