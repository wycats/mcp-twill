...
let surface = NativeToolSurface::builder("project-tools")
    ...
    .build(&registry, McpProtocolTarget::V2026_06_30)?;
```
...
The compiler validates the delivery profile against the exact MCP protocol revision owned by RFC 0015's snapshot. `Legacy2025_11_25` is valid only for `2025-11-25`. Version 1 of `TasksExtension` is valid only for the exact supported extension contract associated with the `2026-06-30` base protocol and extension identifier `io.modelcontextprotocol/tasks`. A later base or extension revision requires a new compiler fixture or snapshot version; lexical date comparison never implies compatibility. On the stateless target, the snapshot revision is server configuration rather than remembered connection state: every request must carry the matching base-protocol observation before this RFC's extension dispatcher becomes reachable.

The checked-in wire corpus at `crates/mcp-twill/tests/fixtures/mcp/tasks/` pins the external contracts separately from Twill's compiled projection. Its `manifest.json` has `formatVersion: 1`, identifies the core specification repository and exact commit containing the `2025-11-25` task schema, identifies the Tasks Extension repository and exact commit containing final SEP-2663 for the `2026-06-30` base protocol, and records the extension identifier. Each payload entry contains its relative path, lowercase SHA-256, source id, derivation (`sourceCopy` or `reviewedVector`), and exact source paths. The manifest does not hash itself, lists every other file exactly once, sorts sources, paths, and source paths lexicographically, and rejects absolute or escaping paths.

These fixtures are external observations, not Twill declarations or generated output. A `sourceCopy` preserves exact source bytes; a `reviewedVector` records an exact request or response case whose relationship to the pinned normative text requires human review. Normal tests verify directory membership and hashes before semantic assertions and never fetch the network. Refresh resolves archived local repositories at the authored commits and produces a visible manifest and fixture diff. Editorial source movement with byte-identical normative inputs may update provenance alone; any semantic change requires a new reviewed compatibility decision and, when it changes accepted or emitted behavior, a new delivery variant, snapshot version, or task runtime contract version according to the ownership rules below. The unversioned extension URL can therefore never silently redefine `TasksExtension` version 1.

### SDK Integration Boundary

The implementation baseline pinned by this RFC is rmcp 1.7.0. Its typed task models represent the legacy task-augmented protocol: its `tools/call`, `tasks/get`, and `tasks/cancel` request variants and result union cannot also represent the extension's incompatible polymorphic call result, inline terminal result, update acknowledgement, and cooperative-cancellation shapes. Twill does not wait for a future SDK release and does not circulate those extension messages as open `serde_json::Value` objects.

`mcp-twill` therefore owns one crate-private `tasks_extension_wire` module containing closed serde types for the exact version-1 extension capability, `tools/call` result alternatives, `tasks/get`, `tasks/update`, and `tasks/cancel` parameters and results. A profile-selected ingress dispatcher sits before rmcp's legacy `ClientRequest` union. On a `TasksExtension` surface it recognizes exactly those methods, deserializes every known member once, and converts immediately to the protocol-neutral runner/store operations below. Missing members, duplicate known members, and wrong kinds fail where their selected method contract requires them to fail. Request parameter objects retain MCP's additive field rule: unknown sibling members are ignored rather than rejected. In particular, a legacy `task` member on a 2026-06-30 `tools/call` is ignored as an unknown field and can never opt that call into task delivery. The version-1 capability object itself remains exact and empty; an extension-owned member inside it is a version mismatch rather than an additive base-request field. On `Disabled` and `Legacy2025_11_25` surfaces the dispatcher delegates to the ordinary rmcp path without making an extension type reachable. The compiled delivery profile and protocol target are the only selector; a request field or extension identifier cannot switch dispatch families.

The rmcp transport hook may hand the ingress dispatcher one raw JSON value, and the final response hook may require one raw result value, but those values exist only at that SDK seam. They never enter the task store, runner, catalog, plans, public Twill API, or application callbacks. Frozen raw-wire fixtures are authoritative over both directions. If a later pinned rmcp release provides validation-semantically identical typed extension models and a profile-safe dispatch hook, Twill may replace the private module with adapters to those types without changing this RFC's public API, snapshot, or wire contract; the SDK release number is not protocol authority.
...
Framework events may record the task profile, public transition kind, and a one-way task-id digest scoped to the server instance. The digest is SHA-256 over `UTF8("io.github.wycats.mcp-twill/task-event") || 0x00 || <256-bit server-instance random salt> || <32 raw task-id bytes>`; the salt is private, never serialized, and discarded with the instance. Events never record the raw task id, salt, access scope, arguments, context, result body, status message source, store error, or execution capsule. Ordinary command events remain governed by their owning RFC and do not gain task payloads.
...
- The 2026-06-30 base-protocol observation is validated per request before extension dispatch; `server/discover`, prior requests, and request extension members cannot establish or replace it.
- A legacy `task` member on a 2026-06-30 `tools/call` is ignored as an additive unknown field and never affects extension materialization.
...
5. Add the crate-private closed extension wire module and profile-selected pre-legacy ingress dispatcher, then implement request capability parsing, server-directed materialization, inline `tasks/get` outcomes, update acknowledgements, cooperative cancellation, and exact extension wire fixtures.
...
- Raw ingress fixtures prove a `TasksExtension` surface routes `tools/call`, `tasks/get`, `tasks/update`, and `tasks/cancel` through the extension types before rmcp's legacy request union; `Disabled` and legacy surfaces never reach those types. Missing members, duplicate known members, wrong kinds, and non-empty version-1 capability objects fail at that seam. Unknown request-parameter siblings are ignored, including a legacy `task` member on 2026-06-30 `tools/call`; matrices prove none can select task delivery or reach a store, runner, plan, public API, or application callback. Instrumentation proves the SDK's temporary raw request/result values likewise never cross that boundary. Equivalent future rmcp typed models must pass the same frozen fixtures before replacing the private compatibility module.
...
- Event-digest vectors prove stable correlation for one task within one server instance, domain/salt separation across instances, and absence of the raw id and salt from serialized events and framework logs.
