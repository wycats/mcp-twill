...
The common suffix explains host facts such as “conversation identity is supplied out of band.” Operation-specific suffixes handle real host exceptions such as the explicit `session start` recovery entrypoint. Structured operation segments render the direct tool name or grouped tool plus selector for the active surface. Resource-carrier segments render only the carrier token for host-authored prose; active RFC 0016 binding guidance already arrives through the compiled native description and is not duplicated here. General operation-selection advice remains in RFC 0011 rather than being repeated here.
...
For a mapped application tool with an explicit `NativeToolDecl` description, the generated user description preserves that authored base without appending catalog, resource-binding, or host guidance. This keeps user-facing copy compatible with a released host manifest while allowing the model-facing contract to remain complete. When a direct or grouped declaration omits its optional description, the generated user description is instead the exact compiled native snapshot description; RFC 0019 does not reconstruct RFC 0015's fallback base or strip derived guidance from the final text. A generated RFC 0015 framework-help tool likewise has no authored application base, so its complete deterministic native snapshot description is its user description.

The generated model description starts with the exact compiled description carried by the matching native tool snapshot, including any RFC 0011 catalog guidance and RFC 0016 resource-binding guidance that native-surface compilation appended, then appends the common host suffix and optional operation suffix. A `ResourceCarrier` host-guidance segment renders a referenced carrier token for host-authored prose; it does not synthesize or duplicate the native resource-binding section. A model description may therefore intentionally differ from an archived host manifest even when its mapped application's user description remains byte-identical. The derived addition is hash-covered and asserted explicitly rather than treated as fixture drift. Server instructions combine the active surface instructions with `server_prefix`. All text participates in the host adapter hash.
...
Twill acceptance tests live in `crates/mcp-twill/tests/host_adapters.rs`. They use a compact generic server for failure, privacy, transport, and determinism cases. VBL parity first validates RFC 0015's manifest, then compares stable generated contribution fields against `vscode-package.json`, native mappings and schemas against `surface-catalog.json`, and generated presentation against `presentation-vectors.json`. The released user descriptions remain exact fixture observations, while model descriptions preserve the released base copy and add the current compiled catalog and resource-binding guidance before the host suffix. Tests assert that intentional model-facing delta directly rather than rewriting the frozen fixture. New host-profile declarations come from `crates/mcp-twill/tests/support/vbl.rs`. Tests do not launch an installed editor or depend on a sibling repository.
...
5. Complete Twill's owner-local VBL profile, frozen v0.4.9 parity, and the declared model-guidance delta in `host_adapters.rs`.
...
- Display names, the explicitly authored user descriptions for all 27 VBL tools, browser icon, server instructions, and four prompt-reference aliases match the established extension manifest. Each mapped application's model description starts with its exact compiled native snapshot description, including any RFC 0011 and RFC 0016 guidance exactly once, and then appends the declared host suffix; the resulting intentional delta from the frozen v0.4.9 `modelDescription` is asserted explicitly. A mapped tool without an explicit description and a generated framework-help tool both use their complete deterministic native description for user copy and the base of model copy, then append only the declared host suffix to the latter.
