...
For a mapped application tool, the generated user description preserves its authored base description without appending catalog, resource-binding, or host guidance. This keeps the user-facing copy compatible with a released host manifest while allowing the model-facing contract to remain complete. A generated RFC 0015 framework-help tool has no authored application base, so its complete deterministic native snapshot description is also its user description.
...
- Display names, mapped-application user descriptions, browser icon, server instructions, and four prompt-reference aliases match the established extension manifest. Each mapped application's model description starts with its exact compiled native snapshot description, including any RFC 0011 and RFC 0016 guidance exactly once, and then appends the declared host suffix; the resulting intentional delta from the frozen v0.4.9 `modelDescription` is asserted explicitly. A generated framework-help tool instead uses its complete deterministic native description for both user copy and the base of model copy, then appends only the declared host suffix to the latter.
