...
2. enforce every presence edge whose triggering argument was supplied;
3. validate each supplied JSON value against its canonical argument schema;
...
6. derive the invocation fingerprint from the completed plan.

Presence checking reads only argument names. When several edges fail, Twill selects the lexically first triggering argument and then its lexically first missing target. It reports the trigger as `ArgumentSchemaMismatch.argument`, the empty root instance path, `DependentRequired`, and the missing target name as `expected`. No new bound-argument or fingerprint fact is needed: the catalog identity covers the relation, and the plan already binds which arguments are present.
...
The enum order above is also the fixed tie-break order after instance-path depth. `Minimum`, `Maximum`, and `MultipleOf` place the assertion's canonical RFC 8785 integer spelling in `expected`. `DependentRequired` places only the lexically selected missing argument name there. Diagnostics never include the supplied numeric value, the set of present arguments, or a synthesized whole-input object.
...
Acceptance lives in `crates/mcp-twill/tests/argument_schemas.rs`; shared JSON Schema compiler cases are table-driven with RFC 0014's result fixtures. The RFC 0017 source fixture consumes `baseline-tools.json` from RFC 0015's already-landed evidence-only fixture bootstrap; the new Twill argument authoring lives in `crates/mcp-twill/tests/support/vbl.rs` and must reproduce every released per-operation property schema and root fact across all 63 operations. The separately authored screencast presence edges are the one documented adoption addition: they capture the grouped release schema and broker behavior that the ungrouped baseline omitted. The bootstrap supplies only provenance-checked observations, while RFC 0015's later public implementation separately owns native projection comparison with `surface-catalog.json`.
...
- Catalog, help, MCP schema, and fingerprint identity change with semantic constraint or presence-edge changes and remain stable across declaration order. A relation changes no `BoundArg` or `schemaMatch` field beyond the already bound argument inventory.
