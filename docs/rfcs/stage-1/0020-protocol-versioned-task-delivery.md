...
The checked-in wire corpus at `crates/mcp-twill/tests/fixtures/mcp/tasks/` pins the external contracts separately from Twill's compiled projection. Its `manifest.json` has `formatVersion: 2` and records four exact sources: legacy MCP `2025-11-25` at commit `38c84e9f93ad191d9eb26d92b945d17bd0efcaf3`, the reviewed pre-tag core `2026-07-28` candidate at commit `31eefec6b979b09ab2092490e2271c0eb38ccd38`, the official core `2026-07-28` tag peeled to commit `5f5440bb26a62e2cf3440b92da5a667efa03b267`, and final SEP-2663 plus the Tasks Extension normative contract at commit `8966bea9c4f4e6d71060cc8284a539086e9e234f` in the canonical `modelcontextprotocol/ext-tasks` repository. It also records the extension identifier. Final core source copies are authoritative; the candidate remains as reviewed lineage for the reconciliation vector described below. Each payload entry contains its relative path, lowercase SHA-256, derivation (`sourceCopy` or `reviewedVector`), and one or more source records with exact source ids and normalized relative paths. Multi-authority vectors therefore record both the Tasks Extension behavior and the final core error identity they inherit. The manifest does not hash itself, lists every other file exactly once, sorts sources, paths, and source records lexicographically, and rejects absolute or escaping paths.

These fixtures are external observations, not Twill declarations or generated output. A `sourceCopy` preserves exact source bytes; a `reviewedVector` records an exact request, response, or source-reconciliation case whose relationship to the pinned normative text requires human review. Normal tests verify directory membership and hashes before semantic assertions and never fetch the network. Refresh resolves archived local repositories at the authored commits and produces a visible manifest and fixture diff. The official tag and its versioned normative bytes are release authority; the pre-tag candidate is retained only to make the reviewed candidate-to-final delta reproducible.

The manifest's `finalRelease` object is exactly `{"tag":"2026-07-28","peeledCommit":"5f5440bb26a62e2cf3440b92da5a667efa03b267"}`. The importer accepts that seal only when the exact tag peels to that commit, every frozen final source has its reviewed hash, and the five candidate-to-final inputs match the closed reconciliation policy. Public stateless construction authenticates the exact canonical manifest bytes and verifies every frozen payload against the manifest before treating the release evidence as sealed. Four documentation inputs differ only in versioned links or Markdown layout. The schema input renames `SubscriptionsListenResultMeta` to `SubscriptionsListenResultMetaObject` and adds the `SubscriptionsListenResultResponse` envelope. Twill's version-1 task contract is polling-only, never advertises task subscriptions, and treats `subscriptions/listen` as an unsupported method, so this final schema delta changes no public API, protocol runtime semantics, native surface identity, task-delivery declaration, or task-runtime contract version. It only satisfies the already-declared public serving gate. The checked-in `core-final-reconciliation.json` records the exact candidate/final paths, hashes, classifications, sealed serving gate, and unchanged Twill contract.

The offline `cargo xtask validate-mcp-task-release` command independently rejects an absent or malformed object, a non-canonical tag or peel, any fixture/hash mismatch, any candidate-to-final difference outside the closed reconciliation policy, and any attempted release whose enabled protocol target is not that sealed revision. The crate publication workflow must pass that command. The same checked-in seal gates the public adapter: with the authenticated final evidence present, `CliMcpServer::into_stateless_service` serves the already-reviewed `V2026_07_28` contract; crate-private tests inject an equivalent unsealed evidence source and require the additive construction error `FrameworkError::ProtocolReleaseUnsealed` before any task store, access provider, binder, authorizer, bridge, handler, or transport runs. There is no public override. A future normative change requires design review before any fixture, accepted behavior, delivery variant, snapshot version, task runtime contract, or release changes. The unversioned extension URL can therefore never silently redefine `TasksExtension` version 1.
...
pub struct StatelessMcpHttpBody { /* private immediate or SSE body */ }
...
{
    type Response = http::Response<StatelessMcpHttpBody>;
...
`StatelessMcpHttpService` receives one complete request byte body and returns a framework-owned `StatelessMcpHttpBody`. The public body implements `http_body::Body<Data = bytes::Bytes, Error = std::convert::Infallible>`. It yields one data frame for an immediate JSON response or incrementally yields Server-Sent Event frames for request-related progress and the final response. Dropping the returned body closes that request's response stream and signals transport cancellation; dropping the service future before it produces the body has the same effect. Protocol, authorization, application, and handler failures are encoded in the HTTP/JSON-RPC body and never escape through Tower's or the body's `Error` channel. The service and body fields and raw protocol request/response types remain private. The service never creates an rmcp session or accepts `initialize`.

For Streamable HTTP, the service reads transport headers directly from the incoming `http::Request<bytes::Bytes>`; no rmcp request extension or session carrier participates. Before any base or extension request dispatch, it extracts the body protocol-version observation, validates the origin and content contract, requires exactly one `MCP-Protocol-Version` equal to that observation, and requires exactly one textual `Mcp-Method` equal to the decoded JSON-RPC `method`. It additionally requires exactly one `Mcp-Name` for `tools/call` and `prompts/get`, or for `resources/read`; when the exact lowercase `=?base64?...?=` sentinel is present, Twill strictly decodes standard Base64 and UTF-8 before comparing the value to `params.name` or `params.uri`. A stateless tool may preserve the final core's typed `x-mcp-header` property annotation in its input schema. Stateless construction rejects an empty, non-token, case-insensitively duplicate, non-primitive, or non-statically-reachable annotation. For `tools/call`, each recognized non-null annotated argument requires exactly one matching `Mcp-Param-{Name}` header; Twill applies the final core's plain/Base64 decoding, safe-integer, and exact-value rules, while an absent or null argument requires the header to be absent. An omitted `arguments` member is the empty object for this comparison and for ordinary dispatch. Unknown parameter headers are ignored. Notifications require the protocol header but do not require `Mcp-Method`, `Mcp-Name`, or parameter headers and receive no response body. Missing, repeated, malformed, undecodable, or unequal required routing headers return HTTP 400 with final core `-32020` / `HeaderMismatch` before complete request-metadata validation, route lookup, task creation, access policy, planning, or application work. Header names compare case-insensitively and decoded values compare case-sensitively. Once a well-formed body protocol version and any required matching HTTP header have been established, a version other than the compiled `2026-07-28` target returns HTTP 400 with final core `-32022` / `UnsupportedProtocolVersionError`, whose data is exactly `{"supported":["2026-07-28"],"requested":"<observed version>"}`. After header, metadata, and version validation, an unsupported JSON-RPC method returns exact `-32601` / `Method not found`; Streamable HTTP uses status 404 and stdio has no HTTP status. Every one of these failures precedes task creation, access policy, planning, or application work. `serve_stdio` otherwise applies the same body dispatcher to newline-delimited JSON-RPC. Clones of the HTTP service share the one finalized server instance and its RFC 0020 runtime pair.

These spellings are authoritative in the official core `2026-07-28` tag peeled to `5f5440bb26a62e2cf3440b92da5a667efa03b267` and agree with the reviewed candidate lineage at `31eefec6b979b09ab2092490e2271c0eb38ccd38`: `HeaderMismatch` is `-32020`, `MissingRequiredClientCapability` is `-32021`, `UnsupportedProtocolVersionError` is `-32022`, and standard `Mcp-Name` accepts the strict Base64 sentinel. The final core allocation governs the shared missing-capability error identity while the Tasks Extension continues to govern the exact `requiredCapabilities.extensions["io.modelcontextprotocol/tasks"] = {}` payload.

`CliMcpServer::into_stateless_service` succeeds only for a native surface compiled as `V2026_07_28` with the checked-in final-release seal. An otherwise valid target returns `FrameworkError::ProtocolReleaseUnsealed`; this construction-only variant has the fixed display text `protocol release evidence is not sealed` and never enters a protocol response. Effect-lane and `V2025_11_25` adapters retain the existing rmcp `ServiceExt::serve` construction path and negotiated behavior. Attempting either serving family with the other compiled target fails before publication or request processing and invokes no task store, access provider, binder, authorizer, bridge, or handler. Existing callers that compiled a provisional `V2026_06_30` surface migrate to `V2026_07_28` and consume the finalized server through `into_stateless_service`; the legacy serving path remains source-compatible.
...
Extension negotiation begins only after the base ingress has validated the request's required `io.modelcontextprotocol/protocolVersion` and `io.modelcontextprotocol/clientCapabilities` observations and, for Streamable HTTP, its matching `MCP-Protocol-Version` header against the compiled `2026-07-28` target. Known final-core client capability members retain their open-object extension points but must have their declared object shapes: `elicitation`, `roots`, and `sampling` are objects; known elicitation and sampling members are objects; and every entry under `experimental` or `extensions` is an object. Unknown capabilities and unknown members inside open known capability objects remain accepted. `io.modelcontextprotocol/clientInfo` is optional; when present, it validates the complete `Implementation` shape, including its optional textual members and icon records, remains descriptive only, and never changes routing or authority. Optional `io.modelcontextprotocol/logLevel` accepts only the final core's declared levels, and optional `progressToken` is a string or exact JSON integer. Unknown metadata siblings remain available to their owners. The dispatcher then validates the capabilities object's `extensions["io.modelcontextprotocol/tasks"]` member as the exact supported empty capability object. Missing or malformed standardized metadata is invalid parameters even for an otherwise unknown request method, a well-formed unsupported observation returns exact `-32022` / `UnsupportedProtocolVersionError`, and an unequal HTTP routing header takes precedence as exact `-32020` / `HeaderMismatch`; each creates no task. A malformed extension capability is invalid parameters at the same pre-task boundary. The server advertises the same fixed empty extension capability through `server/discover`; discovery never establishes remembered client capability or changes the compiled target. Its server-wide response declares `cacheScope: "public"` and `ttlMs: 0`, allowing shared caching while treating the result as immediately stale because version 1 has no invalidation authority. Successful `tools/list`, `resources/list`, `prompts/list`, and `resources/read` results carry framework-owned `cacheScope: "private"` and `ttlMs: 0`; Twill permits reuse only within one authorization context and supplies no freshness guarantee. Every successful result carries framework-owned `_meta["io.modelcontextprotocol/serverInfo"]`; discovery has no duplicate top-level `serverInfo`, unrelated result metadata is preserved, and application-supplied cache values or reserved server identity cannot override the finalized values.
...
Every key under `clientCapabilities.extensions` additionally follows the final core's `_meta` key grammar with its mandatory prefix. An unprefixed extension identifier is malformed standardized metadata and fails at the same pre-task invalid-parameters boundary. The raw ingress parser rejects duplicate standardized request-metadata members before converting `_meta` into its private value representation.

An integer mirrored through `Mcp-Param-*` matches only its canonical decimal spelling; exponent, leading-zero, and explicit-plus spellings fail even when they denote the same mathematical value. Empty-body notification handling applies only after the id-less body has been validated as a JSON-RPC notification envelope with exact `jsonrpc: "2.0"` and a textual method; a malformed id-less object receives the ordinary JSON-RPC error body.
...
| `Required` | `-32021` missing required client capability | task result |
...
For `tasks/get`, `tasks/update`, and `tasks/cancel` over Streamable HTTP, the client routes the request with exactly one `Mcp-Name` header whose decoded value equals the already-decoded `params.taskId`. `StatelessMcpHttpService` reads that header from the incoming `http::Request` parts alongside the standard `Mcp-Method` validation, after decoding the body but before request-local capability parsing, access-scope derivation, or store access. A missing, repeated, non-text, undecodable, or unequal task name returns HTTP 400 with final core `-32020` / `HeaderMismatch` and performs no capability, access-provider, or store operation. No rmcp request extension or session carrier participates. Stdio and other non-HTTP transports perform no `Mcp-Name` check for extension task methods.
...
Dropping an ordinary outer request future drops its prepared invocation. RFC 0019 generated-host cancellation is one such ordinary-request boundary: it never maps to `tasks/cancel`, invokes a task sidecar, or leaves retained task state.

The stateless stdio service owns core `notifications/cancelled` independently from task cancellation. It continues reading while requests execute and keeps one private in-flight map from each live JSON-RPC request id to that request's cancellation token; a second live request with the same id is invalid before dispatch. A well-formed notification for a live id signals that token, suppresses every later response or progress message for the request, and emits no response of its own. A malformed notification or an unknown, completed, or already-cancelled id is ignored. The optional caller reason is never copied into framework logs, events, diagnostics, task status, or application context. Streamable HTTP instead treats closure of that request's response stream or dropping its service future as the cancellation signal and does not expect `notifications/cancelled`.

For immediate execution and for task-selected work before atomic task creation, either transport signal drops prepared authority and prevents later dispatch or task creation. After either task profile's atomic creation point, cancellation of the outer creation request suppresses only that request's remaining transport output; it does not cancel the independent runner or act as `tasks/cancel`. An extension task has independent server-instance lifetime. A legacy task remains owned by its negotiated connection scope: losing one response stream does nothing, while ending that complete session drops its records and signals any framework-owned runner. Application work that already accepted dispatch remains subject to the ordinary best-effort cancellation and no-rollback boundary.
...
- Final core source copies and the exact final tag peel are release authority; the pinned candidate remains only as reproducible lineage for the closed reconciliation vector, and any difference outside that reviewed vector fails the seal.
- Streamable HTTP validates `MCP-Protocol-Version`, `Mcp-Method`, and every applicable base or extension `Mcp-Name` directly from the incoming request against the decoded body before dispatch.
- After those transport checks, every unsupported Streamable HTTP method returns HTTP 404 with exact JSON-RPC `-32601` / `Method not found`; stdio returns the same JSON-RPC error without an HTTP status.
- Stateless stdio processes core cancellation notifications through its private live-request map; HTTP cancellation is response-stream closure, and neither signal becomes extension `tasks/cancel`.
...
1. Land the frozen legacy/candidate/final-core/extension corpus, importer/check mode, exact final-release seal, closed candidate-to-final reconciliation vector, offline release validator, and tamper/reproduction tests. Enable public stateless construction only under that authenticated checked-in evidence.
2. Extract the shipped legacy task lifecycle and fixtures from the current effect-lane rmcp adapter into a protocol-profile module without changing released effect-lane behavior. RFC 0015 contributes only its already-landed protocol-neutral task-support declarations and surface compiler.
3. Add `TaskDeliveryDecl`, `CompiledTaskDelivery`, the surface builder/snapshot accessors, exact profile/protocol validation, canonical snapshot projection, and surface-hash fixtures through RFC 0015's existing types. Keep `TaskSupportSpec` authoring and group homogeneity in RFC 0015; introduce no provisional or parallel surface API.
4. Introduce the shared private runner, authority-bearing execution capsule, stored execution outcome, explicit task store, access policy, atomic transitions, retention, and redacted errors.
5. Correct or update the legacy rmcp `CreateTaskResult._meta` model and pass raw 2025-11-25 wire fixtures.
6. Add the Twill-owned stateless stdio/HTTP serving adapters, crate-private closed base and extension wire modules, and profile-selected pre-legacy dispatcher; then implement request capability parsing, server-directed materialization, inline `tasks/get` outcomes, update acknowledgements, cooperative cancellation, and exact raw-wire fixtures.
7. Compose tasks with the already-landed workspace, conversation-identity, permission, confirmation, ambient-resource, result, event, and surface APIs; move RFC 0016's prepared binding authority into the task capsule; prove RFC 0019 generated hosts use only ordinary fallback in version 1; then run the complete lifecycle and migration suites.

### Acceptance Tests

Acceptance lives in `crates/mcp-twill/tests/tasks.rs` and the public stateless HTTP matrix in `crates/mcp-twill/tests/stateless.rs`. RFC 0015 retains only surface-compilation cases and delegates task lifecycle vectors here.
...
- The MCP task fixture manifest verifies exact legacy, candidate, final-core, and extension repository commits; the final tag and peel; protocol revision; SEP identity; extension identifier; derivation kinds; directory membership; source paths; and payload checksums before any wire assertion. Tampered, extra, missing, partially refreshed, or path-escaping fixtures fail closed; normal tests require no network or sibling checkout. Reviewed vectors remain visibly distinct from exact source copies, and changing the pinned extension source without changing the frozen wire corpus is accepted only as a provenance-only review. The exact reconciliation vector proves the four documentation inputs differ only in versioned links or layout and the schema input differs only in the subscription-listen envelope/meta rename; every other synthetic normative delta fails the importer and offline release-input command. Missing or malformed final evidence keeps crate publication closed.
- Frozen canonical snapshots prove exact disabled, legacy, and extension delivery members, capabilities, tool metadata, protocol revision, runtime contract version, record bound, typed accessors, and surface hashes. A legacy profile/revision mismatch rejects before publication. Stateless fixtures distinguish a missing or malformed body protocol observation, a matching but unsupported body/header version, and an unequal Streamable HTTP protocol header: they return invalid parameters, exact `-32022` / `UnsupportedProtocolVersionError` with `supported` and `requested`, and exact `-32020` / `HeaderMismatch`, respectively, before extension dispatch or task creation. A synthetic runtime-contract increment changes the surface hash. Codec fixtures prove that one runtime version has one exact writer byte language and decoder acceptance set; changing either requires the increment before a new server can address records under its surface identity.
- Public construction coverage proves the checked-in final seal lets `into_stateless_service` accept only a finalized native `V2026_07_28` adapter after authenticating the exact manifest and every frozen payload, while a crate-private unsealed-evidence fixture returns exact `ProtocolReleaseUnsealed` without invoking private sidecars. The resulting service exposes `serve_stdio` plus `into_http_service`; the HTTP service returns a public `StatelessMcpHttpBody` implementing `http_body::Body<Data = bytes::Bytes, Error = Infallible>`. Immediate responses produce one data frame, SSE responses deliver progress before the final response without buffering, and dropping either the pending service future or returned body signals request cancellation. Effect-lane and legacy targets remain rejected. Existing legacy `.serve(...)` fixtures remain source-compatible, while compile fixtures migrate the provisional `V2026_06_30` name and serving call. Raw ingress fixtures prove the stateless service routes base methods plus extension `tools/call`, `tasks/get`, `tasks/update`, and `tasks/cancel` through Twill's closed types before rmcp's legacy request union; `Disabled` never reaches extension lifecycle types and legacy surfaces never reach either stateless module. HTTP matrices cover missing, repeated, malformed, case-varied, matching, and mismatched `MCP-Protocol-Version` and `Mcp-Method` for every supported request, notification omission of `Mcp-Method`, strict plain or Base64-decoded `Mcp-Name` mappings for `tools/call`, `resources/read`, and `prompts/get`, and complete `x-mcp-header` annotation and `Mcp-Param-*` validation; request header failures return HTTP 400 and `HeaderMismatch` before complete metadata validation, notification header failures return HTTP 400 with an empty body, and both precede method or application instrumentation, while an equal well-formed unsupported body/header version returns HTTP 400 and exact `UnsupportedProtocolVersionError`. Public acceptance includes argument-free `tools/call` with omitted `arguments`. Complete standardized request-metadata matrices reject malformed client implementation/icons, log level, progress token, and nested capability shapes while preserving unknown metadata, capability names, and open-object members. Successful public cacheable-result matrices prove the framework-owned cache fields across all four supported methods, while a focused spoof fixture proves application values cannot replace them. Separate method matrices prove every unsupported base or profile-disabled extension method, including final-core `subscriptions/listen`, returns HTTP 404 with exact `-32601` after transport validation and before task, access, or application instrumentation. Missing members, duplicate known members, wrong kinds, and non-empty version-1 capability objects fail at the private wire seam. Unknown request-parameter siblings are ignored, including a legacy `task` member on a `2026-07-28` `tools/call`; matrices prove none can select task delivery or reach a store, runner, plan, public API, or application callback. Instrumentation proves temporary raw request/result values likewise never cross the private adapter boundary. Equivalent future rmcp typed models must pass the same frozen fixtures before replacing either private compatibility module.
- Capability and method inventories for both legacy and extension surfaces omit task listing. A raw `tasks/list` call returns exact `-32601` / `Method not found` before task access, storage, or application instrumentation fires; Streamable HTTP uses status 404 and stdio has no HTTP status. The profiles' exact-id operations remain available under their declared access rules.
...
- Every `tasks/get`, `tasks/update`, and `tasks/cancel` fixture independently covers present, absent, malformed, and context-injected extension capability. Missing capability returns exact `-32021` required-capability data and malformed capability returns invalid parameters before access-provider or store instrumentation fires. `tasks/result` always returns `-32601` on the extension surface.
- Streamable HTTP extension fixtures require exactly one textual `Mcp-Method` equal to the body method and one textual `Mcp-Name` equal to `params.taskId` for each task method, read directly from the incoming HTTP request. Missing, repeated, malformed, undecodable, and unequal task names return HTTP 400 with `-32020` / `HeaderMismatch` before capability parsing, access, or storage; stdio fixtures need no task-name header. Redaction probes prove neither header nor raw task id enters framework projections or logs.
...
- Extension support matrices cover `Forbidden`, both optional policies, and `Required` with and without capability; missing required support returns exact `-32021` before task creation.
...
- Stateless cancellation fixtures run concurrent stdio requests, reject duplicate live ids, cancel one id without affecting another, suppress every later response/progress message, ignore malformed/unknown/completed notifications, and prove the caller reason is non-disclosing. Matching HTTP fixtures cancel by dropping the response stream/future and never require a cancellation notification. Races on both transports prove cancellation before atomic task creation leaves no record, while cancellation after creation suppresses only the outer response and leaves the runner reachable through the task methods.
