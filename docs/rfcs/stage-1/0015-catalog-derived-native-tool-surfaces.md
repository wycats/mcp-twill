...
Deferred delivery exposes the same need for one surface compiler. A direct tool retains one command's `TaskSupportSpec`, while a grouped or effect-lane tool represents several operations through one public MCP entry point. Advertising one task contract for operations that disagree would make the public tool schema less authoritative than the catalog. This RFC therefore requires support homogeneity and leaves the mapping into enabled public task-delivery metadata, client/server negotiation, and the state machine to RFC 0020.
...
`direct` changes no command semantics. The tool's input schema is the selected command's argument schema. Its output schema is the command's RFC 0014 success schema, provided the compiler proves every accepted root value is an object representable as MCP `structuredContent`; a root `$ref` or `oneOf` remains valid when every reachable branch is object-only. Version 1 rejects any direct result branch that can accept an array, scalar, string, or null rather than wrapping or weakening it. Its annotations derive from that command's effects and idempotency. The operation's protocol-neutral task support remains in the compiled operation view and surface identity; this owner-local ordinary-delivery profile advertises the public tool as task-forbidden until RFC 0020 installs a delivery profile. Calling the tool plans and dispatches the command by operation id; no command string is constructed or reparsed.
...
impl NativeToolSurfaceBuilder {
    ...
    pub fn group_description_dialect(
        self,
        dialect: NativeGroupDescriptionDialect,
    ) -> Self;
...
Presentation omission has one exact fallback rather than an implied “group summary” field. A direct tool's final display title is its authored `title` or the command summary; a group's is its authored `title` or the exact public tool name. A direct leading description is its authored `description` or the command description. A grouped leading description is its authored `description` or `Select one operation with `<selector>`.` using the actual selector name. Under the default `CatalogDerived` group-description dialect, Twill then appends derived selected-member, `use_when`, alternative, and fallback guidance in the fixed projection order. Authored title/description replaces only its stated leading slot, and every final string participates in the surface hash. RFC 0018 derives the stored generic presentation defaults from this final display title; neither an adapter nor a generated host repeats title casing or fallback prose.

`AuthoredVerbatim` is an explicit compatibility dialect for adopting a pre-existing native wire contract whose authored group descriptions must remain byte-for-byte unchanged. It suppresses derived group and resource-binding guidance only when the group has an authored description; a group without one still receives the ordinary catalog-derived fallback and guidance. The dialect is serialized in `NativeToolSurfaceDecl`, retained in the canonical snapshot, and covered by the surface hash. It does not alter catalog help, routing, schemas, effects, results, or any runtime policy, and new surfaces use `CatalogDerived` unless compatibility requirements deliberately select otherwise.
...
This is the initial ownership rule, not a negotiation placeholder. One MCP adapter owns one stable tool list and runtime surface identity for its finalized lifetime. Serving another named surface means constructing another adapter and endpoint/router over the same registry; request metadata cannot select a different schema contract inside one adapter. A stateful legacy connection negotiates the adapter's compiled revision once, while a stateless 2026-07-28 request proves that same revision independently at ingress. Per-request versioning selects whether the request may use the adapter; it never recompiles or swaps the adapter's surface.
...
- every grouped output member resolves through any root `$ref` to either one object schema or a non-empty root `oneOf` whose reachable branches are all objects. The compiler preserves a member-owned root union as one nested member branch and inserts the same surface selector into each object branch. A pre-existing selector is accepted only when required and validation-semantically equal to `{ "type": "string", "const": member_selector }` in every reachable branch where it appears;
...
2. Union the top-level properties from every member schema. Repeated property names and same-name local definitions must have identical canonical catalog schemas. When canonical schemas agree but caller-visible array ordering differs, the group emits the first member's authored native projection for that shared schema rather than treating the ordering difference as a conflict.
3. Set tool-level required fields to the selector plus the intersection of member required sets.
4. Collect RFC 0017 `requiresArguments` edges from every member. For each trigger name used by any member, compare the canonical target set of every member whose active direct schema exposes that trigger—meaning the trigger appears under `properties` in that member's own schema before grouping; a member that exposes the property without an edge contributes the empty set. All such sets must be identical, while a member whose own property map lacks the trigger does not participate. Property absence is therefore distinct from an exposed property with no edge: the former contributes no target set, while the latter contributes an empty set and forces every other participating member to do the same. A mismatch fails grouping because the flat input object cannot condition one shared property name's relationship semantics on the selected member without whole-input composition. Repeated identical edges deduplicate canonically, and every trigger and target must be present in the grouped property union. The compiler reconstructs the common relationship map from catalog edges rather than copying a member schema's `dependentRequired` object. Version 1 emits each non-empty common set under sorted legacy `dependencies`, matching the pinned VBL compatibility surface, while the source catalog retains its draft-2020-12 `dependentRequired` projection.
5. Keep `additionalProperties: false`.
6. Preserve each property's nested composition and copy its reachable local definitions. Source schemas already reject dead definitions. Same-name canonical definitions deduplicate; a same-name semantic disagreement fails grouping rather than renaming caller-visible schema components.
7. Do not attach a union to the command's whole input object.

After deserialization, the adapter removes the selector, resolves the member operation, and sends the remaining map to that command's ordinary binder. The binder enforces selected-member required fields and RFC 0017 presence edges, rejects member-inapplicable fields as unknown arguments, and performs all named-union, workspace, resource, permission, and request-context checks. The legacy keyword is therefore a protocol-bound spelling of an already enforced catalog fact, not a surface-only validator or a second input authority. A later native snapshot version may adopt another protocol spelling only with explicit fixture and hash changes.

### Output Schema Projection

Direct tools use the selected command's full success schema from RFC 0014.

Grouped tools use one discriminated/coalesced output contract. Every member success schema must resolve through any root `$ref` to either one object schema or a non-empty root `oneOf` whose reachable branches are all object-only. For a single-object member, the generator removes any existing selector, canonicalizes the remaining schema, and groups members with identical shapes. It then restores the selector as a required property with a constant for a one-member shape class or an enum in surface declaration order for a multi-member class. A member-owned root `oneOf` remains one nested member branch beneath the generated group union: the compiler performs the same selector check and insertion independently in every object branch and does not coalesce that member's internal state variants with another member's shapes. One resulting shape projects as a single object or preserved member union; multiple shapes project as the generated root `oneOf`.

A pre-existing selector is accepted only when it is required and, after annotation removal, validation-semantically equal to `{ "type": "string", "const": member_selector }` in every reachable object branch where it appears. Runtime result validation proves the application value already agrees before the adapter removes and restores the same value. Every other selector collision fails. Primitive, array, string, scalar, nullable, an empty union, or a union with any non-object branch cannot be grouped because adding a selector would require an undeclared envelope or weakening. Reachable same-name definitions deduplicate only when their complete canonical schemas agree; disagreement fails, and every local reference is checked.

The VBL v0.4.9 audit supports this boundary: every operation placed in a grouped hybrid tool accepts objects exclusively. The 63-operation baseline has root output unions for direct `list_tabs` and for the `screencast_stop` and `screencast_status` durable-state results. The latter two are member-owned object unions. The 27-tool hybrid projection intentionally has seven generated root-level output unions after grouped shape-class discrimination, and its `screencast` branch preserves the two durable-state unions one level below that generated discriminator. These unions occur only on outputs, while all 27 hybrid input schemas retain populated top-level properties and no root `oneOf`.
...
The existing `TaskSupportSpec` declaration gains the derived default marker shown above, satisfying the workspace's warning-denied Clippy policy without a manual derivable implementation. `CommandSpec::new` and `CommandBuilder` default to `Optional`. Omitted and explicit `Optional` command declarations normalize identically, and `OperationSpec::from_command_spec` copies the value into its `taskSupport` field, where `Optional` is likewise omitted and defaulted on deserialization. Existing operation-catalog bytes therefore remain unchanged, while `Forbidden` and `Required` become authorable through both low-level and ergonomic paths and change catalog/surface identity. The crate-private `TaskSupportSpec::is_optional` serde predicate is shown only to define the omission spelling across the existing catalog/model modules; authors use the enum and the two declaration methods.

The native compiler retains each operation's exact `TaskSupportSpec` in the operation view and surface identity; a native group requires the identical value of all members. Because this owner-local slice serves ordinary delivery only, every generated native MCP tool advertises `Forbidden` task support, and adapter finalization rejects a surface containing `Required` operations. RFC 0020 later maps the retained values into enabled delivery metadata and runtime behavior. The existing effect-lane compiler continues to project the identical support value required of all operations reachable through each generated lane execution tool. Existing builder-authored commands remain `Optional` until an author explicitly chooses another value, so their effect-lane behavior is unchanged; mixed support inside one grouped or effect-lane tool fails serving-surface construction instead of advertising a misleading tool contract. Generated framework-help tools are always `Forbidden` because they perform no command execution.
...
- the selected group-description dialect, including an explicit `AuthoredVerbatim` compatibility choice;
...
- Grouped argument-presence relationships come only from RFC 0017 catalog edges, retain planner enforcement after selector removal, require identical target sets for every shared trigger property, and use the version-1 native snapshot's exact legacy `dependencies` spelling without becoming a second authoring surface.
...
- Native operation views retain each operation's exact task support, and native groups require one identical reachable-operation value. Owner-local ordinary native tools advertise `Forbidden`, surfaces containing `Required` operations fail adapter finalization, existing effect-lane execution tools retain their exact homogeneous projection, mixed task-support tools fail construction, and generated framework help is always `Forbidden`.
...
- A grouped tool exposes a required selector, unioned top-level properties, common required fields, `additionalProperties: false`, property-level unions from RFCs 0008 and 0017, and every representable catalog-declared argument-presence edge under the exact version-1 `dependencies` spelling. Shared trigger names with identical target sets project once; a trigger present in only one member remains scoped by that member's property applicability.
- Group dispatch enforces operation-specific required and unknown fields plus RFC 0017 presence edges through the selected command planner. A missing paired argument fails identically even when a client ignores the schema keyword.
...
- Registration accepts direct root `$ref` and `oneOf` graphs only when every reachable branch is object-only, and rejects direct or grouped roots with any array, scalar, string, or null branch. A grouped member may resolve one root `$ref` to an object or preserve a non-empty member-authored root `oneOf` when every reachable branch is object-only; that member union remains nested beneath the compiler-owned group discriminator. For groups it also rejects every pre-existing selector property except a required validation-semantic match for the generated singleton string/`const` schema in every reachable branch; a matching value is validated before the adapter removes/restores it, so the surface never overwrites application-owned disagreement.
- Registration rejects duplicate names, one operation mapped more than once, selector collisions, dangling operations, incompatible repeated fields, unresolved schema references, a presence edge whose trigger or target is absent from the grouped property union, or differing target sets when two members expose the same trigger name. Fixtures prove that a member exposing the trigger without an edge conflicts with one requiring a target, so a grouped schema never rejects another member's otherwise valid call. Registration also rejects accidental omission under `Complete`, mapped/omitted/dangling inconsistencies under `ExplicitSubset`, and any subset that breaks caller-steering closure. A capability consumer exposed with only self-dependent refresh providers and no exposed bootstrap provider fails that closure check.
...
- Equivalent low-level `CommandSpec::task_support` and mutable `CommandBuilder::task_support` declarations produce byte-identical operation catalogs and surfaces. Omitted and explicit `Optional` preserve the pre-RFC catalog bytes; `Forbidden` and `Required` change catalog/surface identity, while owner-local ordinary native tools advertise `Forbidden` until RFC 0020 maps the retained value into a delivery profile.
- Direct, native-grouped, and effect-lane `Forbidden`, `Optional`, and `Required` fixtures prove exact catalog and compiled-routing values; every pair of differing values within one generated tool is rejected at surface construction. Owner-local ordinary native tools advertise `Forbidden`, required native operations fail adapter finalization, existing effect-lane tools retain exact support, and generated framework help is always `Forbidden`.
...
- A VBL compatibility fixture projects exactly 27 production tools from 63 catalog operations, with the expected direct/group membership, property-schema composition, output discriminators, annotations, and instructions. The grouped `screencast` tool contains the exact symmetric `max_width`/`max_height` `dependencies` object derived from RFC 0017 edges. RFC 0016 acceptance owns the final ambient carrier omission/optionality and resulting complete input-schema parity.
- The VBL fixture's input schemas contain populated top-level properties and no top-level `oneOf`; nested named and schema-constrained arguments retain property-level composition, and the one grouped presence relation remains an ordinary object assertion.
...
- The VBL v0.4.9 baseline has three root output unions: direct `list_tabs` plus the object-only durable-state unions for `screencast_stop` and `screencast_status`. The hybrid projection has exactly seven generated root unions and preserves the two screencast member unions beneath its generated branch; all hybrid input roots remain ordinary objects.
