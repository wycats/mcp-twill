...
- [RFC 0019: Catalog-Derived Host Adapters](stage-1/0019-catalog-derived-host-adapters.md)
...
behavior changes. The official `2026-07-28` tag peeled to
`5f5440bb26a62e2cf3440b92da5a667efa03b267` is release authority; the pinned
pre-tag candidate remains only as reviewed lineage. Four final documentation
inputs differ only in versioned links or layout, and the final schema adds the
subscription-listen envelope/meta rename. RFC 0020 is polling-only and treats
`subscriptions/listen` as unsupported, so the reviewed reconciliation changes
no Twill API, protocol runtime semantics, surface identity, or task contract.
It only satisfies the already-declared public serving gate. The offline
release-input gate requires that exact final tag, peel, source corpus, and
closed reconciliation vector.
Untagged changes after that release—including routing-value encoding or
error-code reassignment—are not implementation authority; adopting one requires
a newly pinned protocol contract and the corresponding RFC lifecycle.
...
RFC 0020 also owns profile-specific cancellation. Stateless stdio uses core `notifications/cancelled` against a private live-request map, while Streamable HTTP uses response-stream closure; neither transport signal becomes extension `tasks/cancel`. Legacy task cancellation races through one atomic terminal transition. Extension task cancellation acknowledges intent and permits the eventual task to become cancelled, completed, or failed according to the actual winner. In every path, cancellation before realization performs no application work; cancellation after a binder, broker, resolver, or handler accepts work is best-effort and makes no rollback claim. A server-instance task store has one exclusive live finalized-server owner; clones and request routers share that owner, and a persistent backend may be remounted only after the earlier owner exits. This makes a retained working record without a local runner evidence of an orphan rather than evidence of another conforming live worker. Active/active task execution needs a later lease protocol.
...
| RFC 0020 | Final `2026-07-28` release evidence is missing, malformed, or outside the reviewed reconciliation | Construction-only `ProtocolReleaseUnsealed`; no stateless transport, task sidecar, or application callback is created, and crate publication remains closed |
| RFC 0020 | A required Streamable HTTP protocol/method/name header is missing, malformed, repeated, undecodable, or unequal to its body field | HTTP 400 with JSON-RPC `-32020` / `HeaderMismatch`; no method route, task, or application instrumentation runs |
| RFC 0020 | A well-formed request body names a protocol version outside the compiled stateless target, after any required HTTP header has matched it | JSON-RPC `-32022` / `UnsupportedProtocolVersionError` with exact `supported` and `requested` data; HTTP uses status 400, and no extension dispatch, task, or application instrumentation runs |
...
| RFC 0020 | An extension task operation omits or malforms its request-local capability | JSON-RPC `-32021` with exact required-capability data for omission, or invalid parameters for malformed capability; HTTP uses status 400, and no task access or storage runs |
| RFC 0020 | A Streamable HTTP task operation has missing, repeated, malformed, undecodable, or unequal `Mcp-Name` routing | HTTP 400 with JSON-RPC `-32020` / `HeaderMismatch`; no capability parsing, access provider, task lookup, or store operation runs |
...
| 0014 | Every public `CommandRegistry::run*` method changes from `Result<RunResponse>` to `Result<CommandExecutionOutcome>`; direct callers must match success versus application failure. `OutputContract`, `FrameworkEvent`, and public error enums grow additive fields or variants; `ApplicationErrorUse` and `ApplicationErrorSpec` are new declarations. `CommandRegistry::register_result` and `register_dynamic` are additive explicit/generated-server paths. Existing `CommandHandler` signatures, `CommandRegistry::register`, and defaulted RFC 0012 output wrappers remain source-compatible. Command builders that invoked more than one handler installer, repeated output presentation, or paired an explicit application contract with a non-dynamic handler now fail construction instead of using incidental last-write or equality behavior. | Legacy output/event fields and absent capability bindings remain omitted. Commands using one legacy handler and one output declaration preserve success behavior; result-aware commands add declared application outcomes without relabeling framework failures. Safe bounded-runtime messages remain byte-identical; controls and presentation-unsafe scalars use the exact complete-escape encoder, and the declared final bound includes any `…` marker. Model-visible `HandlerFailed` becomes a static empty-details infrastructure response, while direct Rust callers retain the original `FrameworkError`. Editing a result schema, error identity/use, message policy, or recovery edge changes the catalog hash and requires downstream surface and host snapshots to regenerate. | Migrate direct registry callers once; give every command exactly one handler installer and output presentation; use `handle_result` or `register_result` as the sole typed contract authority, or pair `handle_dynamic` with `result_contract` and `register_dynamic` with exactly one explicit contract in `spec.output`'s `application` slot; move portable caller-visible text/cursor facts and intentional handler refusals into the declared application value/error contract. For an RFC 0010 proof refusal, bind the use with `.for_capability(...)` instead of repeating recovery operations or returning legacy `CapabilityDenied`. |
| 0017 | `ArgType::Integer` and diagnostic enums grow variants; `ArgSpec`, `ResourceDecl`, and `BoundArg` grow defaulted fields. `ArgSpec::requires_arguments` plus low-level/builder `requires_argument` methods add catalog-owned presence edges without accepting a whole-input schema. Legacy typed handlers stay on `handle`; constrained typed handlers move to `handle_constrained`, while RFC 0014 handlers select checked extraction automatically. | Missing schema/relationship fields and explicit empty `requiresArguments` preserve coarse schemas, catalog bytes, and fingerprint input. Semantic constraint or presence-edge changes alter the catalog hash; an edge adds no `BoundArg` fact but its canonical selected-command inventory enters every invocation fingerprint directly as the optional `argumentRequirements` preimage member, including bare-registry calls. A changed selected `oneOf` branch alters the canonical `schemaMatch` facts and invocation fingerprint without copying another argument value. Branches reached through local references retain their physical canonical declaration pointers rather than acquiring use-site or validator-generated expansion identities; mismatch-side branch pointers use that same address. `JsonInteger` contributes the exact inline property subschema without root-only Schemars annotations. | Preserve compatibility property schemas byte-for-byte where required; use RFC 0008 for named record unions, RFC 0017 `named_schema` for reusable JSON Schema declarations, `inline_schema` for one-site values, `JsonInteger` when typed extraction must accept the unconstrained integer subset of Twill's JSON number domain, and symmetric `requires_argument` calls for paired optional inputs. Regenerate compiled surfaces and discard affected approval/replay records after any schema or relationship change. |
| 0018 | `CommandSpec` gains defaulted presentation fields and builders; `OperationSpec` gains defaulted `presentation`; `PermissionPreview` gains defaulted `confirmation`; `OperationPresentation` and the remaining presentation types are new. Public struct literals add the defaulted fields or move to constructors. `ResponseEnvelope` retains its fields but gains consistency-validating deserialization. | Missing command declarations, missing/null/known-empty operation presentation, and missing or explicit `null` preview confirmation preserve existing catalog and preview bytes. A historical preview with `requiresConfirmation: true` and no prepared confirmation remains accepted, while every new live require-confirmation preview carries one prepared value. An envelope carrying that value rejects absent or unequal compatibility display text. The active surface otherwise uses deterministic `Running <display-title>` / `Confirmation required` / `Run <display-title>?` fallbacks. A declaration changes catalog identity and the exact `presentationContract` invocation-fingerprint member; changing a surface-owned fallback changes the surface hash and complete served fingerprint. Reordering disjoint cases preserves selected copy but intentionally changes serialized/help order and identity. Presentation never changes authorization. Each host callback renders from one immutable logical argument snapshot; a separated invocation callback requires an explicit host value-equivalence contract before UI can stand in for approval. Rust and generated TypeScript share the frozen ECMAScript `TrimString` table and the exact quote-inclusive 256-scalar truncation budget. | Add defaults to affected public struct literals or use constructors; move portable invocation/confirmation switches into declarations; keep host trigger policy outside the command. Keep authored case order intentional, and discard approval/replay records after adopting, reordering, or changing command presentation or the active surface fallback contract. |
| 0015 | `CommandSpec` gains a defaulted task-support field and low-level/builder setters; `InvocationPlan::raw_command` becomes `Option<String>` and `InvocationOrigin` requires exhaustive handling; plan/runtime identity structs and framework error enums grow fields/variants. Existing `CliMcpServer` constructors keep the effect-lane default; native builders, base-authorizer configuration, and operation-id execution are additive, and native compilation requires an explicit `McpProtocolTarget`. The provisional `McpProtocolTarget::V2026_06_30` spelling is renamed to `V2026_07_28` before the corresponding task-delivery implementation. Mixed `TaskSupportSpec` values inside one generated tool and success schemas that can accept a non-object now fail surface construction. | Omitted and explicit `Optional` command declarations preserve existing operation-catalog bytes and behavior; `Forbidden`/`Required` are explicit adoption. Existing command-template/effect-lane plan wire shapes remain unchanged. Omitting the new finalizing-builder authorizer slot retains `DefaultPermissionAuthorizer`; a supplied object remains private and unhashed. Native success retains matching object `structuredContent` and compact text; application/framework tool errors omit `structuredContent` and carry compact text so the success-only `outputSchema` remains truthful. Every invocation fingerprint gains the exact origin/serving-surface member, intentionally invalidating earlier approval/replay records; native surfaces carry a separate protocol-bound compiled identity including RFC 0020 delivery projection. Convenience server constructors remain limited to synthesized defaults and reject a surface requiring a bridge or extension task runtime. | Update direct plan consumers and replace `V2026_06_30` with `V2026_07_28`; declare non-default task support where required; make every generated tool's reachable operations task-support-homogeneous; keep scalar/array result contracts on CLI or generated-host surfaces unless a later native projection declares a truthful wrapper; discard pre-suite approval/replay records; compile through the single `build(&registry, target)` boundary; choose one native surface per adapter; build omission sets with `explicit_subset`; seed serialized declarations through `builder_from`; use the finalizing server builder whenever confirmation, task delivery, or authorization needs a private adapter sidecar; configure at most one base authorizer and configure a bridge only for a `Bridge` route. |
| 0016 | RFC 0012 extractor traits gain default methods, so existing implementors remain source-compatible. `CommandSpec` and `OperationSpec` gain defaulted optional-resource fields; RFC 0015 native surface declarations, `InvocationPlan`, `PermissionPreview`, and `FrameworkEvent` gain defaulted binding-fact fields, while exported `PlanFacts` gains the corresponding event-construction field. `CommandRegistry::with_resolver_with_errors` and `ServerBuilder::resolver_with_errors` are additive low-level/ergonomic paths; new binders, resolver failures, and framework-error variants are additive. Public struct literals and exhaustive enum matches must account for those additions. | Missing/empty optional-resource and binding-fact fields remain omitted. Omitted and explicit argument-binding defaults compile to one normalized native surface/hash. Existing effect-lane, bare-registry, and argument-bound native resource execution remains the default. An ambient override or `missing_as` mapping changes native surface identity; the selected argument/ambient/absent source and any ambient private digest change the complete invocation fingerprint without adding the raw identity or reference to a plan, preview, or event. | Add ambient binding only to a native surface and let generated hosts consume that compiled snapshot. Fresh fluent authoring uses `bind_resource` to declare the mode and attach its binder atomically; a declaration loaded through `builder_from` uses `attach_resource_binder` so runtime setup never restates serialized policy. Keep effect-lane and bare-registry calls argument-bound. Use `resolver_with_errors` or `with_resolver_with_errors` when domain failures belong to an application result contract. Use `missing_as` only for a code already declared by every required consumer whose static summary, empty details, and full recovery are valid for structural absence; use a binder or resolver for contextual application failures. Discard approval/replay records when adopting ambient binding or changing the selected logical slot. |
| 0020 | RFC 0015 native surface declarations gain defaulted `TaskDeliveryDecl`; native builders gain `task_delivery`, snapshots gain the compiled `task_delivery` accessor, and finalized task-enabled adapters gain one atomic task-runtime pair containing store plus access policy, along with task protocol methods. `CliMcpServer::into_stateless_service`, `StatelessMcpService::{serve_stdio, into_http_service}`, `StatelessMcpHttpService`, and its `StatelessMcpHttpBody` response implementing `http_body::Body` are additive `V2026_07_28` serving paths; `FrameworkError` gains the construction-only `ProtocolReleaseUnsealed` variant; public matches account for disabled, legacy, and extension compiled delivery views. | Omitted and explicit native `Disabled` normalize identically. Existing effect-lane and `V2025_11_25` constructors retain exact legacy negotiated behavior and serving syntax. A sealed `V2026_07_28` server is consumed into the stateless path and never enters rmcp's legacy initialization or request union; the checked-in final evidence is sealed, while a missing, malformed, or out-of-policy evidence source still fails closed at `into_stateless_service`. The stateless HTTP request remains a complete byte body, while its response is an `http_body::Body` that preserves incremental SSE delivery and response-stream cancellation. Legacy delivery uses a negotiated-connection store; extension adoption is explicit, server-directed, protocol-bound, and uses an exclusively mounted server-instance store plus different result/cancellation semantics. The two scopes are not interchangeable. Runtime task choice does not change invocation fingerprints; an authored native delivery declaration changes surface identity. Retained task ids remain addressable only under the exact compiled surface hash, access mode, and owning scope that created them. Version-1 private records have a fixed one-MiB cap and each mounted runtime admits at most 256 retained records; the latter also bounds framework-owned live runners because creation precedes runner start. An over-cap record candidate becomes the static task-execution failure rather than a truncated result, while full-runtime admission becomes the static task-creation failure without disclosing occupancy. Task lifecycle adds no public `FrameworkEvent` field in version 1. Final-core ingress requires protocol version and capabilities on every request, accepts optional valid `clientInfo`, decodes the strict Base64 `Mcp-Name` sentinel, returns public immediately-stale discovery caching, and writes framework-owned `serverInfo` into successful result metadata. | Keep effect-lane and `V2025_11_25` legacy compatibility version-gated on the existing serving path and account for `ProtocolReleaseUnsealed` in exhaustive error matches. Compile stateless native surfaces with `V2026_07_28`, finalize their adapter, then consume the final-evidence-sealed server through `into_stateless_service` and select `serve_stdio` or `into_http_service`; consume `StatelessMcpHttpBody` through the standard `http_body::Body` contract so SSE progress is not buffered and dropping the body cancels only that request; choose a native extension optional policy and bounded retention through `task_delivery`; inspect only the compiled snapshot accessor; for `TasksExtension`, install one exclusively mounted server-instance store plus capability-id or verified transport-scope access through one `task_runtime` assignment; share it only across clones and request routers of that finalized server, remount persistent storage only after the prior owner exits, and use disjoint namespaces until a future active/active lease protocol exists; require request-local extension capability on every task operation and standard plus extension `Mcp-Name` routing on Streamable HTTP; account atomically for the fixed 256-record namespace capacity and free a slot only on successful removal; drain or accept expiry of retained tasks before a catalog/surface/access migration; never use routing, conversation identity, workspaces, or arguments as task authorization. |
...
| 0014 | `results.rs` | RFC 0015 `native_surfaces.rs`; RFC 0016 `ambient_resources.rs`; RFC 0020 `tasks.rs`; RFC 0019 `host_adapters.rs` |
