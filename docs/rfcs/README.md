...
behavior changes. The pinned pre-tag `2026-07-28` candidate is implementation
authority only while public serving remains sealed; the official final tag and
its versioned normative bytes remain release authority. A byte-identical final
tag peel is a provenance-only update, while any normative delta returns to
design review. The offline
