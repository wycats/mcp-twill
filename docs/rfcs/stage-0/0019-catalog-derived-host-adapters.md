...
For the VBL profile above, those rules deliberately reproduce the released v0.4.8 strings rather than merely their meaning. An unsupported `new_tab` call renders:
...
- Unsupported context allows `help` and rejects other configured operations before dispatch with stable `unsupported_host` output. The VBL fixture renders the exact released v0.4.8 string shown above for every unsupported reason and substitutes only the compiled unprefixed native tool name.
