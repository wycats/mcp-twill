...
- The server exposes a single HTTP endpoint (the **MCP endpoint**) that
  accepts POST.
- The client sends every JSON-RPC request, notification, or response as its
  own HTTP POST.
...
- Server-to-client interactions (sampling, elicitation, roots) are embedded
  in results as input requests per [SEP-2322 (MRTR)][sep-2322].
...
4. The body of the HTTP POST **MUST** be a single JSON-RPC _request_,
   _notification_, or _response_ to a server-initiated input request (see
   [Receiving Messages](#receiving-messages-1)).
5. If the body is a JSON-RPC _notification_ or a _response_ to a
...
- The server **MUST NOT** send independent JSON-RPC _requests_ on this stream.
  ...
  [SEP-2322 (MRTR)][sep-2322], not delivered as separate requests on
...
`inputResponses` (see [Multi Round-Trip Requests][sep-2322]):
...
<Warning>
  ...
  **SHOULD** migrate to [Streamable HTTP](#streamable-http). It is eligible for
  removal in a future revision; see the [deprecated features
